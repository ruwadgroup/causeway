...
import causeway.contrib.dramatiq.cli as cli_module
import causeway.contrib.dramatiq.runtime as runtime_module
import causeway.plugins as plugin_registry
from causeway.contrib.dramatiq import DramatiqAdapter, plugin
from causeway.contrib.dramatiq.cli import cli
...
def stub_broker(monkeypatch: pytest.MonkeyPatch) -> StubBroker:
    ...
    monkeypatch.setattr("causeway.contrib.dramatiq.RedisBroker", _StubFactory)
...
def test_env_list_returns_default_and_csv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert runtime_module._env_list("CAUSEWAY_TEST_MODULES", ("app.tasks",)) == ("app.tasks",)
...
def test_worker_module_exposes_bootstrapped_broker(
    ...
    sys.modules.pop("causeway.contrib.dramatiq.worker", None)

    module = importlib.import_module("causeway.contrib.dramatiq.worker")
...
def test_worker_cli_sets_bootstrap_env_and_dramatiq_args(
    ...
    monkeypatch.setattr("causeway.contrib.dramatiq.cli._run_dramatiq", fake_run)
    runner = CliRunner()
    ...
    assert captured["args"] == [
        ...
        "--queues",
        "emails",
        "causeway.contrib.dramatiq.worker:broker",
    ]
...
def test_main_module_invokes_cli_main(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    sys.modules.pop("causeway.contrib.dramatiq.__main__", None)

    runpy.run_module("causeway.contrib.dramatiq.__main__", run_name="__main__")
