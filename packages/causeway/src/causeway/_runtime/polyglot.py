...
JSON encoding/decoding that matches causeway's snake_case wire format.
...
_SWIFT_PREAMBLE = """\
// AUTO-GENERATED by `causeway`. Do not edit by hand. Re-run `causeway swift`.

import Foundation

public struct CausewayClient {
...
public enum CausewayRPCError: Error {
    case http(status: Int, body: Data)
    case decode(Error)
    case transport(Error)
}

public enum CausewayResult<Success, Failure> {
    case ok(Success)
    case err(Failure)
}
"""
...
def render_swift(ir: AppIR) -> str:
    ...
    parts.append("public extension CausewayClient {\n")
...
def _swift_error_enum(method_name: str, variants: list[str]) -> str:
    ...
    for v in variants:
        out.append(
            f'        case "{v}": return .{_lower_first(v)}(try CausewayClient.decoder.decode'
        )
        out.append(f"({_swift_ident(v)}.self, from: data))\n")
    out.append(
        '        default: throw CausewayRPCError.decode(NSError(domain: "causeway", code: 0))\n'
    )
    out.append("        }\n")
    out.append("    }\n")
    out.append("}\n\n")
...
def _swift_method(route: RouteIR) -> str:
    ...
    body.append(
        '            throw CausewayRPCError.transport(NSError(domain: "causeway", code: 0))\n'
    )
    body.append("        }\n")
    ...
    if has_raises:
        ...
        body.append(
            "        let env = (try? CausewayClient.decoder.decode(_Env.self, from: data)) ?? _Env(ok: true, kind: nil)\n"
        )
        ...
        if success_ty == "Void":
            ...
            body.append(
                f"        return try CausewayClient.decoder.decode({success_ty}.self, from: data)\n"
            )
...
def _swift_stream_method(route: RouteIR, method: str, arg_list: str, ret_ty: str) -> str:
    ...
    out.append(
        f"                                    let event = try CausewayClient.decoder.decode({event_ty}.self, from: raw)\n"
    )
...
// AUTO-GENERATED by `causeway`. Do not edit by hand. Re-run `causeway kotlin`.
...
internal val causewayJson = Json {{ ignoreUnknownKeys = true }}

class CausewayRPCError(val status: Int, val body: String) : RuntimeException("HTTP $status")
...
class CausewayClient(
    ...
    internal suspend fun request(
        ...
        if (status !in 200..299) throw CausewayRPCError(status, text)
        text
    }}
"""
...
def render_kotlin(ir: AppIR, *, package: str = "com.causeway.generated") -> str:
    parts: list[str] = [_KOTLIN_PREAMBLE_TEMPLATE.format(package=package)]
    # Methods live inside the CausewayClient class, so we close it after methods.
...
def write_kotlin(ir: AppIR, out: Path, *, package: str = "com.causeway.generated") -> None:
    out.parent.mkdir(parents=True, exist_ok=True)
...
def _kotlin_method(route: RouteIR) -> str:
    ...
    if body_params:
        if len(body_params) == 1 and not body_params[0].embed:
            payload = _to_camel(body_params[0].name)
            out.append(f"        val body = causewayJson.encodeToString({payload})\n")
        else:
            entries = ", ".join(
                f'"{p.alias}" to causewayJson.encodeToJsonElement({_to_camel(p.name)})'
                for p in body_params
            )
            out.append(
                f"        val body = causewayJson.encodeToString(JsonObject(mapOf({entries})))\n"
            )
    else:
    ...
    if route.response is None:
        ...
        out.append("        val env = causewayJson.parseToJsonElement(resp).jsonObject\n")
        out.append('        val ok = env["ok"]?.jsonPrimitive?.content == "true"\n')
        out.append("        if (ok) {\n")
        out.append(
            f'            return causewayJson.decodeFromJsonElement<{success_ty}>(env["data"]!!)\n'
        )
        out.append("        }\n")
        ...
        for v in route.raises:
            out.append(
                ...
                f"causewayJson.decodeFromJsonElement(err))\n"
            )
        out.append("            else -> throw CausewayRPCError(500, resp)\n")
        out.append("        }\n")
    else:
        success_ty = _kotlin_type(route.response)
        out.append(f"        return causewayJson.decodeFromString<{success_ty}>(resp)\n")
...
def _kotlin_stream_method(route: RouteIR, method: str, arg_list: str) -> str:
    ...
    out.append(
        '                                "error" -> throw CausewayRPCError(status, dataBuf.toString())\n'
    )
    out.append("                                else -> emit(\n")
    out.append(
        f"                                    causewayJson.decodeFromString<{event}>(dataBuf.toString())\n"
    )
    out.append("                                )\n")
    out.append("                            }\n")
    out.append("                        }\n")
